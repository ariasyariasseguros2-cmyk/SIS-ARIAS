        ...
        for _, row in df.iterrows():
            ...
            rows.append(
                [
                    ...
                    _normalize_number(row.get("qualitas")),
...
def _refrescar_comisiones(conn) -> None:
    ...
    try:
        ...
        sql_insert = """
            ...
            UNION ALL
            SELECT p.id_producto, c.id_compania, t.qualitas, t.factor
            FROM comisiones_temp t
            JOIN productos p ON p.nombre = t.producto
            JOIN companias c ON c.nombre = 'QUALITAS'
            WHERE t.qualitas IS NOT NULL
        """
