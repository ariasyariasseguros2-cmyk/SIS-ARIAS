...
def _extract_poliza_v3(text: str) -> Optional[str]:
    dash = r"[-–—]"
...
def parse_rimac_generales(text: str) -> Dict[str, str]:
    low = text.lower()
    pol = _extract_poliza_v3(text) or ""
    ...
    try:
        print("[rimac_v3] numero_poliza:", pol)
        print("[rimac_v3] recibo:", recibo)
        print("[rimac_v3] contratante:", contratante if contratante else "(vacio)")
        if item.get("moneda"):
            print("[rimac_v3] moneda:", item.get("moneda"))
        if item.get("prima_neta"):
            print("[rimac_v3] prima_neta:", item.get("prima_neta"))
        if item.get("prima_comercial"):
            print("[rimac_v3] prima_comercial:", item.get("prima_comercial"))
        if item.get("prima_comercial_igv"):
            print("[rimac_v3] prima_total(+IGV):", item.get("prima_comercial_igv"))
        if item.get("inicio_vigencia") or item.get("vencimiento"):
            print("[rimac_v3] vigencia:", item.get("inicio_vigencia"), "-", item.get("vencimiento"))
        if item.get("fecha_emision"):
            print("[rimac_v3] fecha_emision:", item.get("fecha_emision"))
        if item.get("fecha_vencimiento"):
            print("[rimac_v3] fecha_vencimiento:", item.get("fecha_vencimiento"))
    except Exception:
        pass
