...
def parse_crecer_vidaley_pocos_datos(text: str) -> Dict[str, str]:
    ...
    item['prima_comercial'] = _money(_find(r"Prima comercial\s*:?\s*S?/?\s*(\(?\s*(?:[-−–—]\s*)?[0-9.,]+\s*\)?)", text))
    
    # 7. Prima Comercial + IGV: S/ 118.00
    item['prima_comercial_igv'] = _money(_find(r"Prima Comercial \+ IGV\s*:?\s*S?/?\s*(\(?\s*(?:[-−–—]\s*)?[0-9.,]+\s*\)?)", text))
