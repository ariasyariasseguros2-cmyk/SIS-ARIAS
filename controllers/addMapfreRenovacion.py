...
def parse_mapfre_renovacion(text: str) -> Dict[str, str]:
    """
    Parser para PDFs Mapfre Renovación (incluye variantes EPS).
    ...
    print("[parse_mapfre renovacion] texto normalizado:", flat)
    print("item renovacion", item)
    ...
    # Colectivo Asegurado: tomar el texto entre la etiqueta y "Forma de Pago" o "Asegurados"
    # Soporta abreviación "Colectivo Aseg.:"
    cfrag = _find(r"Colectivo\s+Aseg(?:urado|\.)?\s*:\s*(.+?)\s+(?:Forma\s+de\s+Pago|Asegurados|Beneficiarios|Inicio\s+de\s+Vigencia)", flat)
    item["colectivo_asegurado"] = None
    if cfrag:
        # Intentar buscar mayúsculas
        ...
        for cand in reversed(caps):
            if not re.search(r"\d", cand):
                ...
                break
        # Si no halló caps limpias, usar todo el fragmento limpio
        if not item["colectivo_asegurado"]:
             item["colectivo_asegurado"] = cfrag.strip()

    # Fallbacks si el bloque anterior no funciona
    if not item.get("colectivo_asegurado"):
        item["colectivo_asegurado"] = (
            _find(r"Colectivo\s+Aseg(?:urado|\.)?\s*:\s*([A-ZÁÉÍÓÚÑ0-9 \-\.]+)", text)
