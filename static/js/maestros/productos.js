document.addEventListener('DOMContentLoaded', function(){
    try{
        ...
        async function loadRamos(){
            try{
                const res = await fetch('/api/maestros/ramos?per_page=all');
                const data = await res.json();
                const rows = (data && data.rows) || data || [];
                selectRamo.innerHTML = '';
                (rows||[]).forEach(r=>{ const opt = document.createElement('option'); opt.value = r.idRamo || r.id || r.id; opt.textContent = r.nombre || r.marca || r.nombre; selectRamo.appendChild(opt); });
        ...
        async function load(){
            try{
                ...
                const res = await fetch(`/api/maestros/productos?page=${currentPage}&per_page=${perQuery}`);
                const data = await res.json();
                const rows = (data && data.rows) || data || [];
                tableBody.innerHTML = '';
            ...
            }catch(err){ console.error('load productos error', err); }
        }
        ...
        load();
    }catch(e){ console.error('productos.js init error', e); }
});
