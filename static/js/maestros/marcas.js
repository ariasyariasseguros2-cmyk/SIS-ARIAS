document.addEventListener('DOMContentLoaded', function(){
    try{
        const tableBody = document.querySelector('#table-marcas tbody');
        if(!tableBody) return;
        ...
        async function load(){
            try{
                ...
                const res = await fetch(`/api/maestros/marcas?page=${currentPage}&per_page=${perQuery}`);
                ...
                document.querySelectorAll('.btn-del').forEach(b=>b.addEventListener('click', async (e)=>{ try{ if(!confirm('Eliminar este registro?')) return; const id = e.target.dataset.id; await fetch('/api/maestros/marcas/'+id, { method: 'DELETE' }); load(); }catch(err){ console.error(err); alert('Error eliminando'); } }));
            ...
            }catch(err){ console.error('load marcas error', err); }
        }
        ...
        if(saveBtn) saveBtn.addEventListener('click', async ()=>{
            try{ const nombre = (document.getElementById('input-nombre')||{}).value||''; if(!nombre.trim()){ alert('Nombre requerido'); return; } await fetch('/api/maestros/marcas', { method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ nombre }) }); modal.hide(); load(); }catch(err){ console.error(err); alert('Error guardando'); }
        });

        load();
    }catch(e){ console.error('marcas.js init error', e); }
});
