document.addEventListener('DOMContentLoaded', function(){
    try{
        const tableBody = document.querySelector('#table-ramos tbody');
        if(!tableBody) return;
        ...
        async function load(){
            try{
                ...
                const res = await fetch(`/api/maestros/ramos?page=${currentPage}&per_page=${perQuery}`);
                const data = await res.json();
                const rows = (data && data.rows) || data || [];
                tableBody.innerHTML = '';
                (rows||[]).forEach(r=>{ const tr=document.createElement('tr'); tr.innerHTML = `<td>${r.id}</td><td>${r.nombre}</td><td>${r.abreviacion||''}</td><td>${r.codigo||''}</td><td>${r.grupo||''}</td><td>${r.estado||''}</td><td><button class="btn btn-sm btn-danger btn-del" data-id="${r.id}">Eliminar</button></td>`; tableBody.appendChild(tr); });
                document.querySelectorAll('.btn-del').forEach(b=>b.addEventListener('click', async (e)=>{ try{ if(!confirm('Eliminar este registro?')) return; const id = e.target.dataset.id; await fetch('/api/maestros/ramos/'+id, { method: 'DELETE' }); load(); }catch(err){ console.error(err); alert('Error eliminando'); } }));
            ...
            }catch(err){ console.error('load ramos error', err); }
        }
        ...
        if(saveBtn) saveBtn.addEventListener('click', async ()=>{
            try{
                ...
                const abreviacion = (document.getElementById('input-abreviacion')||{}).value||'';
                ...
                await fetch('/api/maestros/ramos', { method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ nombre, abreviacion, codigo, grupo }) });
                modal.hide(); load();
            }catch(err){ console.error(err); alert('Error guardando'); }
        });

        load();
    }catch(e){ console.error('ramos.js init error', e); }
});
