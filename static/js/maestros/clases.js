document.addEventListener('DOMContentLoaded', function(){
    try {
        const tableBody = document.querySelector('#table-clases tbody');
        if(!tableBody) return;
        ...
        function renderPagination(page, pages){
            ...
            function pageItem(p, label, disabled){
                ...
                li.className = 'page-item' + (p === page ? ' active' : '') + (disabled ? ' disabled' : '');
                const a = document.createElement('a');
                a.className = 'page-link';
                a.href = '#';
                a.dataset.page = p;
                a.textContent = label;
                a.addEventListener('click', (e)=>{ e.preventDefault(); if(!disabled){ currentPage = p; load(); } });
                li.appendChild(a);
                return li;
            }
            ...
            for(let p = start; p <= end; p++) ul.appendChild(pageItem(p, p, false));
            ul.appendChild(pageItem(Math.min(pages, page+1), '»', page >= pages));
        ...
        async function load(){
            try{
                ...
                const res = await fetch(`/api/maestros/clases?page=${currentPage}&per_page=${perQuery}`);
                const data = await res.json();
                const rows = (data && data.rows) || data || [];
                tableBody.innerHTML = '';
                (rows || []).forEach(r => {
                    const tr = document.createElement('tr');
                    tr.innerHTML = `<td>${r.id}</td><td>${r.nombre}</td><td>${r.costo_soat}</td><td>${r.estado}</td><td><button class="btn btn-sm btn-danger btn-del" data-id="${r.id}">Eliminar</button></td>`;
                ...
                document.querySelectorAll('.btn-del').forEach(b=>b.addEventListener('click', async (e)=>{
                    try{
                        ...
                        await fetch('/api/maestros/clases/'+id, { method: 'DELETE' });
                        load();
            ...
            }catch(err){ console.error('load clases error', err); }
        }
        ...
        if(saveBtn) saveBtn.addEventListener('click', async ()=>{
            try{
                ...
                const costoEl = document.getElementById('input-costo');
                const nombre = nombreEl ? nombreEl.value : '';
                const costo = costoEl ? costoEl.value : 0;
                if(!nombre.trim()){ alert('Nombre requerido'); return; }
                await fetch('/api/maestros/clases', { method: 'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ nombre, costo_soat: costo }) });
                modal.hide();
                load();
        ...
        load();
    } catch(e){ console.error('clases.js init error', e); }
});
