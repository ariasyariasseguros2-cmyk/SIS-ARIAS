document.addEventListener('DOMContentLoaded', function() {
    ...
    async function fetchData(inicio, fin) {
        try {
            // Show loading state
            // colspan matches header count (12)
            tableBody.innerHTML = `<tr><td colspan="12" class="text-center py-4 text-muted">Cargando datos...</td></tr>`;
    ...
    function renderTable(data) {
        if (!data || data.length === 0) {
            tableBody.innerHTML = `<tr><td colspan="12" class="text-center text-muted py-4">No se encontraron resultados</td></tr>`;
            return;
        }
