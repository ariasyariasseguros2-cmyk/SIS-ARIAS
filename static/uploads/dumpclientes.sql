...
CREATE TABLE IF NOT EXISTS clientes (
    ...
    -- Nuevos campos solicitados por addcliente.py
...
CREATE PROCEDURE sp_insert_cliente (
    ...
    -- Nuevos parámetros para los campos añadidos
...
INSERT INTO clientes (
    ...
    profesion, referencias_interes, notas
             ...
             p_profesion, p_referencias_interes, p_notas
         );
END$$
DELIMITER ;
...
-- Añadir columnas nuevas en la tabla `clientes` si no existen (seguro para MySQL < 8 y >= 5.7)
-- Para cada columna: comprobamos information_schema y ejecutamos ALTER TABLE sólo si hace falta

-- recibir_notificaciones
SET @exists := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE table_schema = DATABASE() AND table_name = 'clientes' AND column_name = 'recibir_notificaciones');
SET @stmt := IF(@exists = 0, 'ALTER TABLE clientes ADD COLUMN recibir_notificaciones TINYINT(1) DEFAULT 0', 'SELECT 0');
PREPARE s FROM @stmt; EXECUTE s; DEALLOCATE PREPARE s;

-- contacto_nombre
SET @exists := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE table_schema = DATABASE() AND table_name = 'clientes' AND column_name = 'contacto_nombre');
SET @stmt := IF(@exists = 0, 'ALTER TABLE clientes ADD COLUMN contacto_nombre VARCHAR(150) NULL', 'SELECT 0');
PREPARE s FROM @stmt; EXECUTE s; DEALLOCATE PREPARE s;

-- contacto_email
SET @exists := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE table_schema = DATABASE() AND table_name = 'clientes' AND column_name = 'contacto_email');
SET @stmt := IF(@exists = 0, 'ALTER TABLE clientes ADD COLUMN contacto_email VARCHAR(150) NULL', 'SELECT 0');
PREPARE s FROM @stmt; EXECUTE s; DEALLOCATE PREPARE s;

-- contacto_telefono
SET @exists := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE table_schema = DATABASE() AND table_name = 'clientes' AND column_name = 'contacto_telefono');
SET @stmt := IF(@exists = 0, 'ALTER TABLE clientes ADD COLUMN contacto_telefono VARCHAR(20) NULL', 'SELECT 0');
PREPARE s FROM @stmt; EXECUTE s; DEALLOCATE PREPARE s;

-- profesion
SET @exists := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE table_schema = DATABASE() AND table_name = 'clientes' AND column_name = 'profesion');
SET @stmt := IF(@exists = 0, 'ALTER TABLE clientes ADD COLUMN profesion VARCHAR(150) NULL', 'SELECT 0');
PREPARE s FROM @stmt; EXECUTE s; DEALLOCATE PREPARE s;

-- referencias_interes
SET @exists := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE table_schema = DATABASE() AND table_name = 'clientes' AND column_name = 'referencias_interes');
SET @stmt := IF(@exists = 0, 'ALTER TABLE clientes ADD COLUMN referencias_interes TEXT NULL', 'SELECT 0');
PREPARE s FROM @stmt; EXECUTE s; DEALLOCATE PREPARE s;

-- notas
SET @exists := (SELECT COUNT(*) FROM information_schema.COLUMNS WHERE table_schema = DATABASE() AND table_name = 'clientes' AND column_name = 'notas');
SET @stmt := IF(@exists = 0, 'ALTER TABLE clientes ADD COLUMN notas TEXT NULL', 'SELECT 0');
PREPARE s FROM @stmt; EXECUTE s; DEALLOCATE PREPARE s;
